...
class EbayShoppingTest < Test::Unit::TestCase
  ...
  def test_should_raise_timeout_error_if_ebay_timeout_error_response
    EbayShopping::Request.any_instance.expects(:call).returns(@timeout_error_response)
  ...
  def test_should_rescue_timeout_error_and_raise_ebay_timeout_instead
    Net::HTTP.any_instance.expects(:get).raises(Timeout::Error)
    assert_raise(EbayShopping::TimeoutError) { new_ebay_request.response }
  end
  ...
  def test_should_return_end_time_as_time_object
    ...
    assert_equal "Sun Jan 06 22:50:09 GMT 2008", item.end_time.to_s
  end
